...
exec java -cp ${CLASSPATH?} ${DEFAULT_JAVA_OPTS:-} ${JAVA_OPTS} -Dlogback.configurationFile=${LOGBACK_CONFIG?} -Dklient=${KLIENT:-"./klient"} -Dwebapp=${WEBAPP:-"./webapp"} -Di18n=${I18N:-"./i18n"} -Dapplication.name=FPLOS ${STARTUP_CLASS?} ${SERVER_PORT:-8080} $@
