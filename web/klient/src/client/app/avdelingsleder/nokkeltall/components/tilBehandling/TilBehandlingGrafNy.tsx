...
import { OppgaveForDatoGraf } from './TilBehandlingGraf';
...
const fyllInnManglendeDatoerOgSorterEtterDato = (
  ...
  for (let dato = dayjs(periodeStart); dato.isSameOrBefore(periodeSlutt); dato = dato.add(1, 'days')) {
    ...
    koordinater.push(funnetDato ? [funnetDato.x, funnetDato.y] : [dato.toDate(), 0]);
  }
...
export const TilBehandlingGraf: FunctionComponent<OwnProps> = ({
  ...
  const farger = sorterteBehandlingstyper.map((bt) => behandlingstypeFarger[bt]);
    ...
    <Panel>
      <ReactECharts
        ...
        option={{
          ...
          series: sorterteBehandlingstyper
